// Drawer.jsx (Basado en "Tremor Drawer [v0.0.1]")
import * as DrawerPrimitives from "@radix-ui/react-dialog";
import { RiCloseLine } from "@remixicon/react";
import React, { forwardRef } from "react";
import { cx, focusRing } from "@/lib/utils";
import { Button } from "./Button";

export function Drawer(props) {
  return <DrawerPrimitives.Root {...props} />;
}

export const DrawerTrigger = forwardRef(function DrawerTrigger(
  { className, ...props },
  ref
) {
  return (
    <DrawerPrimitives.Trigger ref={ref} className={cx(className)} {...props} />
  );
});

export const DrawerClose = forwardRef(function DrawerClose(
  { className, ...props },
  ref
) {
  return (
    <DrawerPrimitives.Close ref={ref} className={cx(className)} {...props} />
  );
});

export const DrawerPortal = DrawerPrimitives.Portal;

export const DrawerOverlay = forwardRef(function DrawerOverlay(
  { className, ...props },
  forwardedRef
) {
  return (
    <DrawerPrimitives.Overlay
      ref={forwardedRef}
      className={cx(
        "fixed inset-0 z-50 overflow-y-auto bg-black/30",
        "data-[state=open]:animate-dialogOverlayShow data-[state=closed]:animate-hide",
        className
      )}
      style={{
        animationDuration: "400ms",
        animationFillMode: "backwards",
      }}
      {...props}
    />
  );
});

export const DrawerContent = forwardRef(function DrawerContent(
  { className, ...props },
  forwardedRef
) {
  return (
    <DrawerPortal>
      <DrawerOverlay>
        <DrawerPrimitives.Content
          ref={forwardedRef}
          className={cx(
            "fixed inset-y-2 mx-auto flex w-[95vw] flex-1 flex-col overflow-y-auto",
            "rounded-md border border-gray-200 bg-white p-4 shadow-lg focus:outline-none",
            "max-sm:inset-x-2 sm:inset-y-2 sm:right-2 sm:max-w-lg sm:p-6",
            "data-[state=closed]:animate-drawerSlideRightAndFade data-[state=open]:animate-drawerSlideLeftAndFade",
            focusRing,
            className
          )}
          {...props}
        />
      </DrawerOverlay>
    </DrawerPortal>
  );
});

export const DrawerHeader = forwardRef(function DrawerHeader(
  { children, className, ...props },
  ref
) {
  return (
    <div
      ref={ref}
      className="flex items-start justify-between gap-x-4 border-b border-gray-200 pb-4"
      {...props}
    >
      <div className={cx("mt-1 flex flex-col gap-y-1", className)}>
        {children}
      </div>
      <DrawerPrimitives.Close asChild>
        <Button variant="ghost" className="aspect-square p-1 hover:bg-gray-100">
          <RiCloseLine className="size-6" aria-hidden="true" />
        </Button>
      </DrawerPrimitives.Close>
    </div>
  );
});

export const DrawerTitle = forwardRef(function DrawerTitle(
  { className, ...props },
  forwardedRef
) {
  return (
    <DrawerPrimitives.Title
      ref={forwardedRef}
      className={cx("text-base font-semibold text-gray-900", className)}
      {...props}
    />
  );
});

export const DrawerBody = forwardRef(function DrawerBody(
  { className, ...props },
  ref
) {
  return <div ref={ref} className={cx("flex-1 py-4", className)} {...props} />;
});

export const DrawerDescription = forwardRef(function DrawerDescription(
  { className, ...props },
  forwardedRef
) {
  return (
    <DrawerPrimitives.Description
      ref={forwardedRef}
      className={cx("text-gray-500", className)}
      {...props}
    />
  );
});

export function DrawerFooter({ className, ...props }) {
  return (
    <div
      className={cx(
        "flex flex-col-reverse border-t border-gray-200 pt-4 sm:flex-row sm:justify-end sm:space-x-2",
        className
      )}
      {...props}
    />
  );
}
